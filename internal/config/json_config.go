...
func applyJSONConfig(cfg *Config, jc *JSONConfig) {
	...
	applyJSONServerConfig(cfg, jc)
	applyJSONHandlerConfig(cfg, jc)
	applyJSONLoggerConfig(cfg, jc)
	applyJSONRepoConfig(cfg, jc)
	applyJSONDBConfig(cfg, jc)
	applyJSONAuthConfig(cfg, jc)
	applyJSONAuditConfig(cfg, jc)
}

func applyJSONAuditConfig(cfg *Config, jc *JSONConfig) {
	if jc.AuditFile != nil {
...
func applyJSONAuthConfig(cfg *Config, jc *JSONConfig) {
...
func applyJSONDBConfig(cfg *Config, jc *JSONConfig) {
...
func applyJSONRepoConfig(cfg *Config, jc *JSONConfig) {
	if jc.FileStoragePath != nil {
		cfg.Repo.FileStoragePath = *jc.FileStoragePath
	}
}

func applyJSONLoggerConfig(cfg *Config, jc *JSONConfig) {
	if jc.LogLevel != nil {
		cfg.Logger.LogLevel = *jc.LogLevel
	}
}

func applyJSONHandlerConfig(cfg *Config, jc *JSONConfig) {
	if jc.BaseURL != nil {
		cfg.Handler.BaseURL = *jc.BaseURL
	}
}

func applyJSONServerConfig(cfg *Config, jc *JSONConfig) {
